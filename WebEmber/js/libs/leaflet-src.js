...
L.TileLayer = L.Class.extend({
	...
	_initContainer: function () {
		// TO-DO Victor: I added if bellow
		if (this._map) {
...
L.Marker = L.Class.extend({
	...
	initialize: function (latlng, options) {
		L.setOptions(this, options);
		// TO-DO quick fix causing NaN when projecting latitude with leaflet
		latlng[0] = Number(latlng[0]).toFixed(5);
		latlng[1] = Number(latlng[1]).toFixed(5);
...
L.Path = L.Path.extend({
	...
	_initStyle: function () {
		...
		if (this.options.arrows) {
			this._path.setAttribute('marker-start', "url(#head)");
			this._path.setAttribute('marker-end', "url(#head)");
			this._path.setAttribute('marker-mid', "url(#head)");
		}
		this._updateStyle();
	},
...
L.Map.include({
	_initPathRoot: function () {
		if (!this._pathRoot) {
			...
			var arrows = "<defs><marker id='mid' orient='auto' markerWidth='2' markerHeight='4' refX='0.1' refY='1'>" +
				    "<!-- triangle pointing right (+x) --><path d='M0,0 V2 L1,1 Z' fill='orange'/></marker>"+
				    "<marker id='head' viewBox='0 0 10 10' refX='0' refY='5' markerUnits='strokeWidth'" +
				     "markerWidth='4' markerHeight='3' orient='auto'><path d='M 0 0 L 10 5 L 0 10 z' /></marker></defs>";

			this._pathRoot.appendChild($(arrows)[0]);
		}
	},
...
L.Control.Layers = L.Control.extend({
	...
	removeOverlayFromMap : function (layer) {
		var i, input, obj,
		...
		for (i = 0; i < inputsLen; i++) {
			...
			if (obj.name == layer.name && obj.layer) {
	...
	addOverlayToMap : function (layer) {
		var i, input, obj,
		    inputs = this._form.getElementsByTagName('input'),
		    inputsLen = inputs.length;
	...
	_addLayer: function (layer, name, overlay) {
		...
		this._nameToLayer[name] = this._layers[id];
		...
		for (i = 0; i < inputsLen; i++) {
			...
			if (input.checked && !this._map.hasLayer(obj.layer)) {
				...
				if (!obj.overlay) {
					...
					this._map.fire('overlayadd', {layer: obj, clicked : true});
				}
			} else if (!input.checked && this._map.hasLayer(obj.layer)) {
				this._map.removeLayer(obj.layer);
				this._map.fire('overlayremove', {layer: obj, clicked : true});
			}
		}

		if (baseLayer) {
			this._map.setZoom(this._map.getZoom());
			this._map.fire('baselayerchange', {layer: baseLayer, clicked : true});
		}
