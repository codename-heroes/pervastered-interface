...
links.Timeline.prototype.repaintFrame = function() {
    ...
    if (!dom.content) {
        ...
        dom.content.style.overflow = "visible";
        dom.frame.appendChild(dom.content);



        var timenavLine = document.createElement("DIV");
        var timenavIndicator = document.createElement("DIV");
        timenavLine.className = "timenav-line";
        timenavIndicator.className = "timenav-indicator";

        var timenavTop  = document.createElement("DIV");
        timenavTop.className = "timeline-frame-top";
        dom.frameTop = timenavTop;

        dom.content.appendChild(timenavLine);
        dom.content.appendChild(timenavTop);
        dom.content.appendChild(timenavIndicator);
...
links.Timeline.prototype.repaintAxis = function() {
    ...
    if (!axis.frame) {
        ...
        axis.frame.className = "timeline-axis-frame";
...
links.Timeline.prototype.repaintItems = function() {
    ...
    if (!frame) {
        ...
        dom.content.insertBefore(frame, dom.content.firstChild);
        frame.className="timeline-items-frame";
...
links.Timeline.prototype.repaintCurrentTime = function() {
    ...
    if (!dom.currentTime) {
        ...
        currentTime.style.top = "20px";
        currentTime.style.height = "80%";
        currentTime.style.zIndex = 420;
...
links.Timeline.prototype.createCustomTime = function(groupId) {
    var options = this.options,
        dom = this.dom,
        size = this.size,
        customTimesGroups = dom.customTimesGroups,
        customTimes = dom.customTimes;


    if (!customTimes) {
        dom.customTimes = new Array();
        dom.customTimesGroups = new Array();
        

        customTimesGroups = dom.customTimesGroups;
        customTimes = dom.customTimes;
    }
    // TO-DO put this as a paramter. Right now we just support one timeline
    if (!groupId && customTimes.length > 0) return;
    ...
    if (!groupId) {
        groupId = customTimesGroups.length;
    }
    customTime.setAttribute("marker-group", groupId);

    var color;
    if (customTimesGroups.length <= groupId) {
        customTimesGroups[groupId] = new Array();
        color = '#'+Math.floor(Math.random()*16777215).toString(16);
    } else {
        color = customTimesGroups[groupId][0].style.backgroundColor;
    }
    customTimesGroups[groupId][customTimesGroups[groupId].length] = customTime;

    var x = this.timeToScreen(this.customTime),
        visible = (x > -size.contentWidth && x < 2 * size.contentWidth);
    var nearX = x+10;
    var timeNearX = this.screenToTime(nearX);
    customTime.style.display = visible ? '' : 'none';
    customTime.style.left = nearX + "px";
    customTime.title = timeNearX;

    customTime.style.backgroundColor = color;

    dom.customTimes[dom.customTimes.length] = customTime;


    var drag = document.createElement("DIV");
    drag.className = "marker-drag";
    ...
    var markerBackground = document.createElement("DIV");
    markerBackground.className = "marker-background";
    markerBackground.style.background = color;
    markerBackground.style.borderColor = color;
    var markerNumber = document.createElement("span");
    markerNumber.className = "marker-number";
    marker.appendChild(markerNumber);
    marker.appendChild(markerBackground);

    var posX = -(groupId%4)*29,
        posY = -Math.floor(groupId/4)*45;
    marker.style.backgroundPosition = posX + "px " + posY +"px";
    customTime.appendChild(marker);
    customTime.appendChild(drag);

}

links.Timeline.prototype.deleteCustomTime = function () {
...
links.Timeline.prototype.repaintCustomTimeRange = function() {
    var dom = this.dom;
    ...
    if (this.selectedRange) {
        if (!dom.range){
            ...
            range.style.opacity = "0.2";
            range.style.zIndex = 100;
            dom.content.appendChild(range);
            dom.range = range;
        }

        var group = dom.customTimesGroups[this.selectedRange];
        var first = group[0];
        var last = group[group.length-1];
        dom.range.style.left = first.style.left;
        dom.range.style.backgroundColor = first.style.backgroundColor;
        dom.range.style.width = (parseFloat(last.style.left) - parseFloat(first.style.left))+"px";
    } else if (dom.range) {
        dom.range.parentNode.removeChild(dom.range);
        dom.range = undefined;
    }
}
...
links.Timeline.prototype.repaintCustomTime = function() {
    ...
    if (!dom.customTime) {
        ...
        this.customTime = (this.options["start"].getTime() + this.options["end"].getTime())/2;
        this.createCustomTime();
        this.customTime -= (this.options["end"].getTime() - this.options["start"].getTime())/4;
        this.createCustomTime(this.dom.customTime.getAttribute("marker-group"));
    }

    for (var i = 0; i < dom.customTimes.length; ++i) {
        var customTime = dom.customTimes[i];
        var x = this.timeToScreen(new Date(customTime.getAttribute("title"))),
            visible = (x > -size.contentWidth && x < 2 * size.contentWidth);
        customTime.style.display = visible ? '' : 'none';
        customTime.style.left = x + "px";
    }

    for (var i= 0; i < dom.customTimesGroups.length; ++i) {
        var group = dom.customTimesGroups[i];
        // TO-DO just sort when moving an item

        group.sort(function(a,b) {return new Date(a.title) > new Date(b.title);});
        for (var j=0; j < group.length; ++j) {
            var customTime = group[j];

            var markerNumber = customTime.getElementsByClassName("marker-number")[0];
            markerNumber.innerHTML = j;
        }
    }

    this.repaintCustomTimeRange();
};
...
links.Timeline.prototype.recalcConversion = function() {
    ...
    //this.conversion.offset = this.conversion.offset - links.Timeline.getAbsoluteLeft(this.dom.content)/this.conversion.factor;
};
...
links.Timeline.prototype.onMouseDown = function(event) {
    ...
    if (!params.touchDown) {
        if (options["orientation"] == "vertical") {
            params.mouseY = event.clientX;
            params.mouseX = event.clientY;
        } else {
        ...
        if (options["orientation"] == "vertical") {
            params.mouseX = event.targetTouches[0].clientY;
            params.mouseY = event.targetTouches[0].clientX;
        } else {
    ...
    for (var i in dom.customTimes) {
        var customTime = dom.customTimes[i];
        params.customTime = (params.target === customTime ||
            params.target.parentNode === customTime) ?
            new Date(customTime.getAttribute("title")) :
            undefined;

        dom.customTime = customTime;
        if (params.customTime) break;
    }
    params.moveRange = (params.target.className === "marker-range");

    params.addCustomTime = (params.target === dom.frameTop);
...
links.Timeline.prototype.onMouseMove = function (event) {
    ...
    if (!params.touchDown) {
        ...
        if (this.options["orientation"] == "vertical") {
            mouseY = event.targetTouches[0].clientX;
            mouseX = event.targetTouches[0].clientY;
        } else {
    ...
    if (params.customTime) {
        ...
        var xnew = x + diffX;
        dom.customTime.title = this.screenToTime(xnew);
        this.customTime = x;
        this.repaintCustomTime();
        // fire a timechange event
        this.trigger('timechange');
    } else if (params.moveRange) {
        var customTimesGroup = dom.customTimesGroups[0];
        params.moveRange = params.mouseX;
        params.mouseX = mouseX;
        for (var i = 0; i < customTimesGroup.length; ++i) {
            var customTime = customTimesGroup[i];
            var x = this.timeToScreen(new Date(customTime.title));
            var xnew = x + diffX;
            customTime.title = this.screenToTime(xnew);
        }
        this.repaintCustomTime();
        this.trigger('customrangemoved');
...
links.Timeline.prototype.onMouseUp = function (event) {
    ...
    if (params.customTime) {
        this.customTime = params.customTime;

        // fire a timechanged event
        if (!params.moved && params.target.className == "timeline-marker") {
            this.createCustomTime(this.dom.customTime.getAttribute("marker-group"));
        } else if (event.toElement.className.indexOf("marker") == -1) {
            this.deleteCustomTime();
        }

        this.trigger('timechanged');
        this.trigger('customrangechanged');
        this.repaintCustomTime();
    } else if (params.moveRange) {
        this.trigger("customrangechanged");
    } else if (params.addCustomTime) {
        if (this.options["orientation"] == "horizontal") this.customTime = this.screenToTime(params.mouseX-params.frameLeft);
        else this.customTime = this.screenToTime(params.mouseX-this.getAbsoluteLeftVertical());
        if (!params.moved) {
            this.createCustomTime();
        }
        this.repaintCustomTime();
...
links.Timeline.prototype.onMouseOver = function(event) {
    ...
    var target = links.Timeline.getTarget(event);
    if(dom.mouseOverTarget && target != dom.mouseOverTarget) {
        // mouseOut
        if (target.className != "marker-range" && dom.mouseOverTarget.className == "marker-drag") {
            this.selectedRange = undefined;
            this.repaintCustomTimeRange();
        }

        // mouseIn
        if (target.className == "marker-drag") {
            this.selectedRange = target.parentNode.getAttribute("marker-group");
            this.repaintCustomTimeRange();
        }
    }
    dom.mouseOverTarget = target;
...
links.Timeline.prototype.trigger = function (event) {
    ...
    switch (event) {
        ...
        case 'timechanged':
            ...
            break;
        case 'customrangechanged':
            var group = this.dom.customTimesGroups[0];
            var dates = [];
            for (var i = 0; i < group.length; ++i) {
                var date = group[i];
                dates[i] = new Date(date.getAttribute("title"));
            }
            properties = {
                'dates': dates,
            };
...
links.Timeline.prototype.getTimeRange = function() {
    var result = [],
        timeRanges = this.dom.customTimesGroups[0];

    for (var i = 0; i < timeRanges.length; ++i) {
        result.push(new Date(timeRanges[i].title));
    }
...
links.Timeline.prototype.getAbsoluteLeftVertical = function() {
    var dom = this.dom;
    var left = links.Timeline.getAbsoluteLeft(dom.content);
    var top = links.Timeline.getAbsoluteTop(dom.content);
    return top - dom.content.offsetWidth/2 + dom.content.offsetHeight/2;
};

links.Timeline.prototype.getAbsoluteTopVertical = function() {
    var dom = this.dom;
    var left = links.Timeline.getAbsoluteLeft(dom.content);
    var top = links.Timeline.getAbsoluteTop(dom.content);
    return left - dom.content.offsetHeight/2 + dom.content.offsetWidth/2;
};
